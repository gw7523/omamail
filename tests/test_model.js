...
deepEqual(model.windowPrefs(""), {
  ...
  alwaysShowImages: false, readerShowsSentTime: true, windowOpen: false
})
assert.strictEqual(model.windowPrefs('{"readerShowsSentTime":false}').readerShowsSentTime, false,
  "the sent time can be switched off")
...
// draws: the open mailbox key, or the label whose query is open.
{
  ...
  // The rail draws labels in the order it was given them (the tree's), so
  // the switcher's rows and numbers follow that order too.
  deepEqual(rows[2], { kind: "label", id: "Label_17", name: "todo", icon: "label",
    number: 3, count: 3, selected: true })
  deepEqual(rows[3], { kind: "label", id: "Label_9", name: "Receipts", icon: "label",
    number: 4, count: 0, selected: false })
